...
def render_tab_5(tab):
    if tab == 'tab-5':
        return html.Div([
            dcc.Graph(id='world-map', config={'scrollZoom': False}),
            dcc.Graph(id='suicide-bar-chart'),  # Add the bar chart component
            html.Div(id='output-div')  # Add a div to display the output of click events
        ])
    else:
        return []
...
# Tab-5 Function 02
@app.callback(
    Output('suicide-bar-chart', 'figure'),  
    [Input('world-map', 'clickData')]
)
def update_suicide_bar_chart(clickData):
    if clickData:
        
        country_clicked = clickData['points'][0]['location']

        
        filtered_data = df[df['country'] == country_clicked]

        
        grouped_data = filtered_data.groupby(['year', 'sex'])['suicides_no'].sum().reset_index()



        fig = px.bar(grouped_data, x='year', y='suicides_no', color='sex',
             title=f'Yearly Suicide Trends Across Genders in {country_clicked}',
             labels={'year': 'Year', 'suicides_no': 'Suicide Count', 'sex': 'Gender'},
             barmode='group',
